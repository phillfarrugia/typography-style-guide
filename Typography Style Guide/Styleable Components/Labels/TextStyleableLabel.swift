...
extension TextStyleable where Self: UILabel {
    
    /// Applies the TextStyle to the label
    func applyStyle() {
        if let text = text {
            let attributes = shouldUseParagraphStyle ? textStyle.multilineAttributes : textStyle.attributes
            attributedText = NSAttributedString(string: text, attributes: attributes)
        }
    }
    
    /// Should use Paragraph Style
    var shouldUseParagraphStyle: Bool {
